...
class Run:
    ...
    def load_sv_log_data(self, dir_path, current_playlist, suffix_plus_ext):
        """
    ...
    def load_gt_data(self, dir_path, current_playlist):
        """
        ...
        sv_log_data =  self.load_raw_data(dir_path, current_playlist, suffix_plus_ext='lda_tiny_log.csv')

        try:
            groundtruth = GroundTruth(current_playlist, self.argument.output_path)
            gt_df = groundtruth.generate_gt(dgps_data, mobileye_data, sv_log_data)
    ...
    def generate_coefficient_graph(self):
        ...
        for i in range(len(playlist_df.index)):
            ...
            sv_log_data1 = self.load_sv_log_data(self.log_sv_dir_path_1, current_playlist, suffix_plus_ext='lda_tiny_log.csv')
            sv_log_data2 = self.load_sv_log_data(self.log_sv_dir_path_2, current_playlist, suffix_plus_ext='lda_tiny_log.csv')
            sv_log_data3 = self.load_sv_log_data(self.log_sv_dir_path_3, current_playlist, suffix_plus_ext='lda_tiny_log.csv')
            sv_log_data4 = self.load_sv_log_data(self.log_sv_dir_path_4, current_playlist, suffix_plus_ext='lda_tiny_log.csv')
            ...
            if self.argument.is_gt_plot:
                ...
                gt_data = self.load_gt_data(self.log_sv_dir_path_1, current_playlist)
            else:
                gt_data = self.initial_data()

            # Plot CPP coefficients graph
            if self.argument.is_cpp_plot:
                cpp_data = self.load_sv_log_data(self.log_sv_dir_path_1, current_playlist, suffix_plus_ext='adaf_log.csv')
            else:
